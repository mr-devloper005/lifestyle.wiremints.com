...
export function TaskDetailView({ task, post, related, comments = [] }: { task: TaskKey; post: SitePost; related: SitePost[]; comments?: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  const detailVars = { '--detail-bg': '#ffffff', '--detail-text': '#050505', '--detail-surface': '#ffffff', '--detail-accent': '#ffe66d' } as CSSProperties
    ...
    <section className="bg-white">
      <header className="border-b border-black/20 bg-black text-white">
        <div className="mx-auto max-w-[1180px] px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
          ...
          {summaryText(post) ? <p className="mt-6 max-w-4xl text-xl font-bold leading-8 text-white/68 sm:text-2xl">{summaryText(post)}</p> : null}
        </div>
      </header>

      {images[0] ? (
        <figure className="mx-auto max-w-[1180px] overflow-hidden border-x border-b border-black/15 bg-white">
          <div className="relative aspect-[16/10] max-h-[640px] bg-[#f3f3f3] sm:aspect-[16/8]">
  ...
  return (
    <section className="mt-8">
      ...
      <div className={`mt-4 grid gap-3 ${large ? 'sm:grid-cols-2' : 'grid-cols-2 sm:grid-cols-4'}`}>
        {images.slice(0, large ? 4 : 8).map((image, index) => <img key={`${image}-${index}`} src={image} alt="" className="aspect-[4/3] h-full w-full rounded-[1.4rem] object-cover ring-1 ring-[var(--editable-border)]" />)}
      </div>
    </section>
  )
}
...
function RelatedPanel({ task, post, related, compact = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="rounded-[16px] border border-black/10 bg-white p-5 shadow-[0_12px_30px_rgba(0,0,0,.06)]">
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex min-w-0 gap-3 border-t border-black/15 py-3 transition hover:text-black/60">
      {image && task !== 'sbm' ? <img src={image} alt="" className="h-20 w-20 shrink-0 rounded-[10px] object-cover" /> : <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-[10px] bg-black text-white"><FileText className="h-6 w-6" /></div>}
